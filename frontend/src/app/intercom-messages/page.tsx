...
import { MessageSquare, User, Bot, Clock, Search, Filter, ExternalLink, RefreshCw, Download } from 'lucide-react'
...
export default function IntercomMessagesPage() {
  ...
  const [sortBy, setSortBy] = useState<'created_at' | 'message_index'>('created_at')
  ...
  }, [messages, searchTerm, conversationFilter, authorTypeFilter, partTypeFilter, sortBy, sortOrder])
  ...
  const filterAndSortMessages = () => {
    ...
    if (conversationFilter) {
      filtered = filtered.filter(msg =>
        msg.conversation_id.toLowerCase().includes(conversationFilter.toLowerCase())
      )
    }
  ...
  const exportMessages = () => {
    ...
    const csv = [
      ['Message ID', 'Conversation ID', 'Author Type', 'Author Name', 'Part Type', 'Body', 'Created'],
    ...
    a.href = url
    a.download = `intercom-messages-${new Date().toISOString().split('T')[0]}.csv`
    a.click()
  }
  ...
  const uniqueAuthorTypes = [...new Set(messages.map(msg => msg.author_type).filter(Boolean))]
  const uniquePartTypes = [...new Set(messages.map(msg => msg.part_type).filter(Boolean))]
    ...
    <div className="container mx-auto p-6 max-w-7xl">
      ...
      <div className="space-y-4">
        {paginatedMessages.map((message) => (
          <div key={message.message_id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-4">
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      {message.author_type === 'admin' || message.author_type === 'bot' ? (
                      ...
                      <span className="font-medium text-gray-900">
                        {message.author_name || message.author_email || `${message.author_type} ${message.author_id.slice(-6)}`}
                      </span>

                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        AUTHOR_TYPE_COLORS[message.author_type as keyof typeof AUTHOR_TYPE_COLORS] || 'bg-gray-100 text-gray-800'
                      }`}>
                        {message.author_type}
                      </span>

                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                        PART_TYPE_COLORS[message.part_type as keyof typeof PART_TYPE_COLORS] || 'bg-gray-100 text-gray-800'
                      }`}>
                        {message.part_type}
                      </span>
                    ...
                    <div className="mb-3">
                      {message.body ? (
                        <div className="text-gray-900">
                          {expandedMessage === message.message_id ? (
                            ...
                            <div>
                              ...
                              {message.body.length > 200 && (
                                <button
                                  onClick={() => setExpandedMessage(message.message_id)}
                          ...
                          {expandedMessage === message.message_id && message.body.length > 200 && (
                            <button
                        ...
                        <div className="text-gray-500 italic">No message body</div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-500">
                      <div className="flex items-center gap-4">
                        ...
                        <button
                          onClick={() => setConversationFilter(message.conversation_id)}
      ...
      {filteredMessages.length === 0 && !loading && (
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <p className="text-gray-600 mb-4">
            {searchTerm || conversationFilter || authorTypeFilter !== 'all' || partTypeFilter !== 'all'
