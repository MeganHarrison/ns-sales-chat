...
export default function IntercomDashboardPage() {
  ...
  const fetchDashboardStats = async () => {
    ...
    try {
      ...
      const [conversationsResult, conversationsByStateResult, recentConversationsResult] = await Promise.all([
        supabase.from('intercom_conversations').select('conversation_id', { count: 'exact' }),
        supabase.from('intercom_conversations').select('state', { count: 'exact' }),
        supabase.from('intercom_conversations').select('conversation_id, state, created_at, updated_at')
          .order('updated_at', { ascending: false }).limit(5)
      ])

      // Fetch messages stats
      const [messagesResult, messagesByAuthorResult, recentMessagesResult] = await Promise.all([
        supabase.from('intercom_messages').select('message_id', { count: 'exact' }),
        supabase.from('intercom_messages').select('author_type', { count: 'exact' }),
        supabase.from('intercom_messages').select('message_id, conversation_id, author_type, author_name, created_at, body')
      ...
      // Fetch admins stats
      const [adminsResult, availableAdminsResult, awayAdminsResult, inboxAdminsResult] = await Promise.all([
        supabase.from('intercom_admins').select('admin_id', { count: 'exact' }),
        supabase.from('intercom_admins').select('admin_id', { count: 'exact' }).eq('away_mode_enabled', false),
        supabase.from('intercom_admins').select('admin_id', { count: 'exact' }).eq('away_mode_enabled', true),
        supabase.from('intercom_admins').select('admin_id', { count: 'exact' }).eq('has_inbox_seat', true)
      ])

      // Fetch tags stats
      const [tagsResult, tagUsageResult] = await Promise.all([
        supabase.from('intercom_tags').select('tag_id', { count: 'exact' }),
        supabase.from('intercom_conversation_tags').select('tag_id', { count: 'exact' })
      ])

      // Process conversation states
      const stateGroups = conversationsByStateResult.data?.reduce((acc: any, item: any) => {
        acc[item.state] = (acc[item.state] || 0) + 1
        return acc
      }, {}) || {}

      // Process message author types
      const authorGroups = messagesByAuthorResult.data?.reduce((acc: any, item: any) => {
        acc[item.author_type] = (acc[item.author_type] || 0) + 1
        return acc
      }, {}) || {}

      // Process user types
      const userTypeGroups = usersByTypeResult.data?.reduce((acc: any, item: any) => {
      ...
      const dashboardStats: DashboardStats = {
        ...
        syncStats: {
          lastSyncTime,
          totalDataPoints: (conversationsResult.count || 0) + (messagesResult.count || 0) +
                           (usersResult.count || 0) + (adminsResult.count || 0) + (tagsResult.count || 0)
        }
      }
    ...
    <div className="container mx-auto p-6 max-w-7xl">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 group-hover:shadow-md transition-shadow border-l-4 border-green-500">
            ...
            <div className="mt-3 flex items-center justify-between text-xs">
              <span className="text-blue-600">
                {stats.messages.byAuthorType.find(t => t.author_type === 'user')?.count || 0} from users
              </span>
        ...
        <Link href="/intercom-tags" className="group">
          <div className="bg-white rounded-lg shadow-sm p-6 group-hover:shadow-md transition-shadow border-l-4 border-orange-500">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-gray-900">{stats.tags.total}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Conversation States</h3>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-600">Open</span>
              <span className="text-sm font-medium text-green-600">{stats.conversations.open}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className="bg-green-600 h-2 rounded-full"
                style={{
                  width: stats.conversations.total > 0
                    ? `${(stats.conversations.open / stats.conversations.total) * 100}%`
                    : '0%'
                }}
              ></div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium text-gray-600">{stats.conversations.closed}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                ...
                style={{
                  width: stats.conversations.total > 0
                    ? `${(stats.conversations.closed / stats.conversations.total) * 100}%`
                    : '0%'
                }}
              ></div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium text-red-600">{stats.conversations.unassigned}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                ...
                style={{
                  width: stats.conversations.total > 0
                    ? `${(stats.conversations.unassigned / stats.conversations.total) * 100}%`
                    : '0%'
                }}
              ></div>
            </div>
            ...
            {stats.messages.byAuthorType.map((author, index) => {
              ...
              return (
                <div key={author.author_type}>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-600 capitalize">{author.author_type}</span>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 group-hover:shadow-md transition-shadow">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <div className="font-medium text-green-600">{stats.admins.available}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">Away</div>
                <div className="font-medium text-orange-600">{stats.admins.away}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">With Inbox</div>
                <div className="font-medium text-blue-600">{stats.admins.withInboxSeat}</div>
              </div>
            </div>
          </div>
        </Link>
          ...
          <div className="space-y-2 text-sm">
            {stats.users.byType.map((userType) => (
              <div key={userType.type} className="flex items-center justify-between">
                <span className="text-gray-600 capitalize">{userType.type}</span>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="space-y-3 text-sm">
            <div>
              ...
              <div className="font-medium text-green-600">{stats.syncStats.totalDataPoints.toLocaleString()}</div>
            </div>
            <div>
              <div className="text-xs text-gray-500">Last Updated</div>
              <div className="font-medium">{formatValue(stats.syncStats.lastSyncTime, 'dateTime')}</div>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="space-y-3">
            {stats.conversations.recent.map((conv) => (
              <div key={conv.conversation_id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <div>
                ...
                <span className={`text-xs px-2 py-1 rounded-full ${
                  conv.state === 'open' ? 'bg-green-100 text-green-800' :
                  conv.state === 'closed' ? 'bg-gray-100 text-gray-800' :
            ...
            {stats.messages.recent.slice(0, 5).map((msg) => (
              <div key={msg.message_id} className="p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <div className="text-sm font-medium text-gray-900">
                    {msg.author_name || `${msg.author_type} ${msg.conversation_id.slice(-6)}`}
                  </div>
                  <span className={`text-xs px-2 py-1 rounded-full ${
                    msg.author_type === 'user' ? 'bg-blue-100 text-blue-800' :
                    msg.author_type === 'admin' ? 'bg-purple-100 text-purple-800' :
