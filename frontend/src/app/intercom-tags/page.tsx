...
import { Tag, Hash, Calendar, TrendingUp, RefreshCw, Download, Search } from 'lucide-react'
...
export default function IntercomTagsPage() {
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'created_at':
          ...
          break
        case 'usage':
          const aUsage = tagUsage.find(usage => usage.tag_id === a.tag_id)?.conversation_count || 0
          const bUsage = tagUsage.find(usage => usage.tag_id === b.tag_id)?.conversation_count || 0
  ...
  const totalUsage = tagUsage.reduce((sum, usage) => sum + usage.conversation_count, 0)
  const mostUsedTag = tagUsage.reduce((max, current) =>
    ...
    <div className="container mx-auto p-6 max-w-7xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <p className="text-sm font-medium text-purple-600 truncate">{mostUsedTag.tag_name}</p>
          <p className="text-xs text-gray-500">{mostUsedTag.conversation_count} conversations</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <p className="text-2xl font-bold text-orange-600">{averageUsage.toFixed(1)}</p>
        </div>
      </div>
          ...
          return (
            <div
              ...
              className={`bg-white rounded-lg shadow-sm border-2 transition-all hover:shadow-md cursor-pointer ${
                selectedTags.has(tag.tag_id) ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
              }`}
              ...
              <div className="p-4">
                ...
                <div className="mb-3">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium text-gray-900">{conversationCount}</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                    <div
                      className="bg-green-600 h-2 rounded-full transition-all duration-300"
                      style={{
                        width: totalUsage > 0 ? `${Math.min((conversationCount / totalUsage) * 100 * 10, 100)}%` : '0%'
                      }}
                    ></div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between text-xs text-gray-500 border-t border-gray-100 pt-3">
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      setShowDetails(showDetails === tag.tag_id ? null : tag.tag_id)
                    }}
                ...
                {showDetails === tag.tag_id && (
                  <div className="mt-3 p-3 bg-gray-50 rounded text-xs space-y-2" onClick={(e) => e.stopPropagation()}>
                    <div><strong>Tag ID:</strong> {tag.tag_id}</div>
                    <div><strong>Name:</strong> {tag.name}</div>
                    <div><strong>Created:</strong> {formatValue(tag.created_at, 'dateTime')}</div>
                    <div><strong>Updated:</strong> {formatValue(tag.updated_at, 'dateTime')}</div>
                    <div><strong>Synced:</strong> {formatValue(tag.synced_at, 'dateTime')}</div>
                    <div><strong>Conversations:</strong> {conversationCount}</div>
                    {conversationCount > 0 && (
                      <div><strong>Usage %:</strong> {((conversationCount / totalUsage) * 100).toFixed(1)}%</div>
                    )}
                  </div>
                )}
              </div>
      ...
      {filteredTags.length > 0 && (
        <div className="mt-6 bg-white rounded-lg shadow-sm p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Tag Usage Distribution</h3>
                ...
                <div key={usage.tag_id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm font-medium text-gray-900 truncate">{usage.tag_name}</span>
                      <span className="text-sm text-gray-600">{usage.conversation_count}</span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className="bg-green-600 h-2 rounded-full transition-all duration-300"
                        style={{
                          width: totalUsage > 0 ? `${(usage.conversation_count / totalUsage) * 100}%` : '0%'
                        }}
                      ></div>
                    </div>
                  </div>
