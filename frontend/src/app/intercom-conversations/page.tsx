...
import { MessageCircle, Users, Clock, Filter, Search, Eye, RefreshCw, Download } from 'lucide-react'
...
export default function IntercomConversationsPage() {
  ...
  const [filteredConversations, setFilteredConversations] = useState<IntercomConversation[]>([])
  const [loading, setLoading] = useState(true)
  const [syncing, setSyncing] = useState(false)
  const [searchTerm, setSearchTerm] = useState('')
  const [stateFilter, setStateFilter] = useState('all')
  const [priorityFilter, setPriorityFilter] = useState('all')
  const [sortBy, setSortBy] = useState<'created_at' | 'updated_at' | 'state'>('updated_at')
  ...
  }, [conversations, searchTerm, stateFilter, priorityFilter, sortBy, sortOrder])
  ...
  const selectAllVisible = () => {
    const visibleIds = paginatedConversations.map(conv => conv.conversation_id)
  ...
  const exportConversations = () => {
    const dataToExport = selectedConversations.size > 0
      ? filteredConversations.filter(conv => selectedConversations.has(conv.conversation_id))
      : filteredConversations

    const csv = [
      ['Conversation ID', 'Type', 'State', 'Priority', 'Created', 'Updated', 'Title'],
    ...
    a.href = url
    a.download = `intercom-conversations-${new Date().toISOString().split('T')[0]}.csv`
    a.click()
  }
  ...
  const uniqueStates = [...new Set(conversations.map(conv => conv.state).filter(Boolean))]
  const uniquePriorities = [...new Set(conversations.map(conv => conv.priority).filter(Boolean))]
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  <input
                    type="checkbox"
                    checked={selectedConversations.size === paginatedConversations.length && paginatedConversations.length > 0}
                    onChange={() => selectedConversations.size === paginatedConversations.length ? clearSelection() : selectAllVisible()}
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedConversations.map((conversation) => (
                <tr key={conversation.conversation_id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <input
                      type="checkbox"
                      checked={selectedConversations.has(conversation.conversation_id)}
                      onChange={() => toggleConversationSelection(conversation.conversation_id)}
                      className="h-4 w-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500"
                    />
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                      STATE_COLORS[conversation.state as keyof typeof STATE_COLORS] || 'bg-gray-100 text-gray-800'
                    }`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                      PRIORITY_COLORS[conversation.priority as keyof typeof PRIORITY_COLORS] || 'bg-gray-100 text-gray-800'
                    }`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button
                      onClick={() => setShowDetails(showDetails === conversation.conversation_id ? null : conversation.conversation_id)}
                      className="text-blue-600 hover:text-blue-700 flex items-center gap-1"
                    >
                      <Eye className="h-4 w-4" />
                      {showDetails === conversation.conversation_id ? 'Hide' : 'View'}
                    </button>
                  </td>
                </tr>
              ))}
