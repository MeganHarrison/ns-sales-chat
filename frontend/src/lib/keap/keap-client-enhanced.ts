...
export class KeapClientEnhanced {
  ...
  async getContacts(query?: KeapContactQuery): Promise<KeapListResponse<KeapContact>> {
    const response = await this.request<any>('GET', '/contacts', { params: query });

    // Keap API returns { contacts: [...], count: number }
  ...
  async getOrders(query?: KeapOrderQuery): Promise<KeapListResponse<KeapOrder>> {
    const response = await this.request<any>('GET', '/orders', { params: query });

    // Keap API returns { orders: [...], count: number }
  ...
  async getSubscriptions(query?: KeapSubscriptionQuery): Promise<KeapListResponse<KeapSubscription>> {
    const response = await this.request<any>('GET', '/subscriptions', { params: query });

    // Keap API returns { subscriptions: [...], count: number }
    if (response.subscriptions !== undefined) {
      return {
  ...
  async getProducts(query?: KeapQueryOptions): Promise<KeapListResponse<KeapProduct>> {
    const response = await this.request<any>('GET', '/products', { params: query });

    // Keap API returns { products: [...], count: number }
