 ...
 */
export function formatMoney(money: KeapMoney | number): string {
  // Handle both KeapMoney object and plain number
  if (typeof money === 'number') {
    return `$${money.toFixed(2)}`;
  }
  // Handle KeapMoney object
...
export function calculateTotalRevenue(orders: KeapOrder[]): number {
  return orders.reduce((total, order) => {
    if (order.status === KeapOrderStatus.PAID) {
      // Handle both number and KeapMoney object types
...
export function exportOrdersToCSV(orders: KeapOrder[]): string {
  ...
  const rows = orders.map(order => {
    ...
    const amount = typeof order.total === 'number'
      ? order.total
      : (order.total?.amount || 0);
    const currency = typeof order.total === 'object' && order.total?.currency_code
